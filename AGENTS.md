...
- M1-M4 are complete. Hosted authentication and portfolio/watchlist workflows pass on the emulator; M2 phone and backend-auth testing were user-confirmed. Distinguish those evidence sources. M3 adds coordinated 90-second refresh, quote preservation, local watchlist selection, and lifecycle cancellation. M4 search/chart automated/emulator checks pass and the user confirmed completion after the physical-phone checklist. M5 is now authorized. See `milestones.md`; commit at M5 completion and pause.
