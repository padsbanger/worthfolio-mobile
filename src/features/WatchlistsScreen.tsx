...
function WatchRow({ symbol }: { symbol: string }) {
  const { data, isError, isFetching } = useMarket(symbol, '1D');
  ...
  const quoteStatus = [!online && !demo ? 'Offline' : isError ? 'Refresh unavailable' : isFetching ? 'Refreshing' : null,
...
export function WatchlistsScreen() {
  ...
  return <View style={styles.screen}><DataNotice />
    <FlatList data={current?.symbols ?? []} keyExtractor={symbol => symbol} contentContainerStyle={styles.content}
      refreshControl={<RefreshControl refreshing={result.isRefetching || refresh.refreshing} enabled={online || demo} onRefresh={() => void reload()} tintColor={colors.accent} />}
      ...
      ListEmptyComponent={result.isError && !data ? null : <Status title={!online && !demo && !data ? 'Connect to load watchlists' : result.isPending && !data ? 'Loading watchlists' : current ? 'This list is empty' : 'No watchlists yet'}
