...
export function PriceChart({ observations, currency }: { observations: Observation[]; currency: string }) {
  ...
  return <View style={{ gap: 12 }}>
    <Text style={styles.text} accessibilityLiveRegion="polite">{money(inspected?.close, currency)} · {timestamp(inspected?.time)}</Text>
    <View onLayout={event => setWidth(event.nativeEvent.layout.width)} accessible accessibilityRole="adjustable"
      accessibilityLabel="Price history" accessibilityValue={{ text: `${money(inspected?.close, currency)}, ${timestamp(inspected?.time)}` }}
