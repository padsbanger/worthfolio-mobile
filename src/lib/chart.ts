...
export function chartGeometry(observations: Observation[], width: number, height: number) {
  ...
  if (!sorted.length) return { points: [] as ChartPoint[], path: '', low: 0, high: 0, ticks: [] as { value: number; y: number }[] };
  ...
  const priceY = (price: number) => 8 + (high + margin - price) / (high - low + 2 * margin) * Math.max(0, height - 16);
  const ticks = (high === low ? [low] : [high, (high + low) / 2, low]).map(value => ({ value, y: priceY(value) }));
  const points = sorted.map(p => ({ ...p,
    x: duration ? 8 + (Date.parse(p.time) - start) / duration * Math.max(0, width - 16) : width / 2,
    y: priceY(p.close),
  }));
  return { points, low, high, ticks, path: points.map((p, i) => `${i ? 'L' : 'M'}${p.x.toFixed(2)},${p.y.toFixed(2)}`).join(' ') };
}
